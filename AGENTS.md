...
- **Landmarks**: `app/layout.tsx` owns `<main id="content">` and `<footer>` plus the skip link. There is no site-wide header/navbar; contact links live on the front of the home-page business card, projects on its reverse (flip on click). Pages must NOT render their own `<main>`.
...
- **Security**: headers live in `next.config.ts`. CSP allows inline scripts because rendering is static; switch to nonce-based CSP (`proxy.ts`) if going dynamic.
